...
void runLocalIntensityScalePipeline(const PixelDataDim &tempImageDim, const APRParameters &par, S *cudaImage, S *cudaTemp, S *lstPadded, S *lst2Padded, cudaStream_t aStream) {
    ...
    if (!constant_scale) {
        ...
        if (par.reflect_bc_lis) {
            ...
            runPaddPixels(cudaTemp, lst2Padded, imageSize, paddedImageSize, paddSize, aStream);

            ci = lstPadded; //paddedImage.get();
            ct = lst2Padded; //paddedTemp.get();
            dim = paddedImageSize;
        }
...
void getLocalIntensityScale(PixelData<T> &image, PixelData<T> &temp, const APRParameters &par) {
    ...
    PixelDataDim paddedImageSize = image.getDimension() + maxPaddSize + maxPaddSize;
