                ...
                for(j_ = 0;j_ < j_num;j_++){
                    ...
                    if (!(node_val_pc&1)){
                        ...
                        for(p = 0;p < pc_struct.part_data.get_num_parts(status);p++){
                            ...
                            if( binary_mask.get_part(curr_key) >0){
...
void calc_cc_mesh(Mesh_data<T>& input_mask,T include_val,Mesh_data<uint16_t>& comp_label){
    ...
    int z_num_ = input_mask.z_num;
    int x_num_ = input_mask.x_num;
    int y_num_ = input_mask.y_num;

    timer.verbose_flag = true;


    comp_label.initialize(y_num_,x_num_,z_num_,0);
    ...
    for(int z_ = 0;z_ < z_num_;z_++){
        ...
        for(int x_ = 0;x_ < x_num_;x_++){
            ...
            //the y direction loop however is sparse, and must be accessed accordinagly
            for(int y_ = 0;y_ < y_num_;y_++){


                if(input_mask(y_,x_,z_) == include_val) {
                    //loop over neighbours
                    std::vector<int> neigh_labels;

                    if(x_!=0) {
                        if(comp_label(y_,x_-1,z_) > 0) {
                            neigh_labels.push_back(comp_label(y_,x_-1,z_));
                        }
                    ...
                    if(y_!=0) {
                        if(comp_label(y_-1,x_,z_) > 0) {
                            neigh_labels.push_back(comp_label(y_-1,x_,z_));
                        }
                    ...
                    if(z_!=0) {
                        if(comp_label(y_,x_,z_-1) > 0) {
                            neigh_labels.push_back(comp_label(y_,x_,z_-1));
                        }
                    ...
                    if(neigh_labels.size() == 0){
                        ...
                        comp_label(y_,x_,z_) = uf_make_set(labels);
                        ...
                        comp_label(y_,x_,z_) = neigh_labels[0];
                        ...
                        comp_label(y_,x_,z_) = curr_label;
    ...
    debug_write(comp_label,"comp_label_t");
    ...
    for(int z_ = 0;z_ < z_num_;z_++){
        ...
        for(int x_ = 0;x_ < x_num_;x_++){
            ...
            for(int y_ = 0;y_ < y_num_;y_++){
                ...
                if( comp_label(y_,x_,z_) > 0){

                    int x = uf_find(comp_label(y_,x_,z_) ,labels);
                    if (new_labels[x] == 0) {
                        new_labels[0]++;
                        new_labels[x] = new_labels[0];
                    }

                    comp_label(y_,x_,z_) = new_labels[x];
