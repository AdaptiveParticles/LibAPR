...
public:
    ...
    std::string boundary_cond = "reflecting";
    ...
    inline void set_boundary_conditions_yx(const bool boundary_condition,const std::vector<int>& stencil_half,const std::vector<int> &stencil_shape,PixelData<T>& temp_vec,const uint64_t z){
        ...
        std::string image_file_name =  "temp_vec_debug.tif";
        TiffUtils::saveMeshAsTiffUint16(image_file_name, temp_vec,false);
        ...
        if(boundary_condition == REFLECT_PAD){
            ...
            for (int x = 0; x < stencil_half[1]; ++x) {
                for (int y = stencil_half[0]; y < (temp_vec.y_num - stencil_half[0]); ++y) {
                    temp_vec.at(y,x,z_off) = temp_vec.at(y,x+stencil_half[1],z_off);
                }

            }

//            //first do the x reflection (x_num - 1 -> x_num - 1 - stencil_half)
//            for (int x = 0; x < stencil_half[0]; ++x) {
//
//            }
        ...
        image_file_name =  "temp_vec_debug_after.tif";
        TiffUtils::saveMeshAsTiffUint16(image_file_name, temp_vec,false);
    ...
    template<typename T>
    inline void set_boundary_conditions_z(const bool boundary_condition,const std::vector<int>& stencil_half,PixelData<T>& temp_vec,const uint64_t z){
...
void APRFilter::convolve(APR<ImageType> &apr, PixelData<T>& stencil, ExtraParticleData<S> &particle_input, ExtraParticleData<R> &particle_output) {
    ...
    apr.apr_tree.fill_tree_mean(apr, apr.apr_tree, particle_input, tree_data);
...
inline void update_same_level(const uint64_t level,
                              const uint64_t z,const uint64_t x,APRIterator &apr_iterator,PixelData<float> &temp_vec,
    ...
    for (apr_iterator.set_new_lzx(level, z, x);
        ...
        temp_vec.mesh[apr_iterator.y() + stencil_half[0] + mesh_offset] = inputParticles[apr_iterator];

        if(boundary_cond){
            if(apr_iterator.y() < stencil_half[0]){
                temp_vec.mesh[apr_iterator.y() + mesh_offset] = inputParticles[apr_iterator];

                //corners
                if(x < stencil_half[1]) {

                    temp_vec.mesh[apr_iterator.y() + mesh_offset - stencil_half[1]*y_num_m] = inputParticles[apr_iterator];

                }else if(x > (x_num_m - 2*stencil_half[1] - 2)){

                    temp_vec.mesh[apr_iterator.y() + mesh_offset + 2*stencil_half[1]*y_num_m] = inputParticles[apr_iterator];

                }

            }else if(apr_iterator.y() > (y_num_m - 2*stencil_half[0] - 2)){
                temp_vec.mesh[apr_iterator.y() + 2*stencil_half[0] + mesh_offset] = inputParticles[apr_iterator];

                //corners
                if(x < stencil_half[1]) {

                    temp_vec.mesh[apr_iterator.y() + mesh_offset - stencil_half[1]*y_num_m + 2*stencil_half[0]] = inputParticles[apr_iterator];

                }else if(x > (x_num_m - 2*stencil_half[1] - 2)){

                    temp_vec.mesh[apr_iterator.y() + mesh_offset + 2*stencil_half[1]*y_num_m + 2*stencil_half[0]] = inputParticles[apr_iterator];
...
#pragma omp parallel for schedule(dynamic) private(x) firstprivate(apr_iterator)
    ...
    for (x = 0; x < apr_iterator.spatial_index_x_max(level); ++x) {
        ...
        update_same_level(level,z,x,apr_iterator,temp_vec,inputParticles,stencil_shape, stencil_half, y_num_m, x_num_m,boundary_cond,mesh_offset);

        //boundary conditions
        if(boundary_cond){
            if(x < stencil_half[1]) {

                mesh_offset = (x) * y_num_m + x_num_m * y_num_m * (z % stencil_shape[2]);
                update_same_level(level, z, x, apr_iterator, temp_vec, inputParticles, stencil_shape, stencil_half,
                                  y_num_m, x_num_m, boundary_cond, mesh_offset);
            }else if(x > (x_num_m - 2*stencil_half[1] - 2)){

                mesh_offset = (x + 2*stencil_half[1]) * y_num_m + x_num_m * y_num_m * (z % stencil_shape[2]);
                update_same_level(level,z,x,apr_iterator,temp_vec,inputParticles,stencil_shape, stencil_half, y_num_m, x_num_m,boundary_cond,mesh_offset);
            }
        ...
        for (x = 0; x < apr.spatial_index_x_max(level); ++x) {
            ...
            for (apr_iterator.set_new_lzx(level - 1, z / 2, x / 2);
                ...
                temp_vec.at(2 * apr_iterator.y() + stencil_half[0], x + stencil_half[1],
                            z % stencil_shape[2]) = inputParticles[apr_iterator];//particleData[apr_iterator];
...
template<typename ImageType, typename S,typename R>
void create_test_particles_equiv(APR<ImageType>& apr,const std::vector<PixelData<float>> &stencil_vec,ExtraParticleData<S>& input_particles
                                 ,ExtraParticleData<R>& output_particles){

    ExtraParticleData<float> part_tree;

    output_particles.init(apr.total_number_particles());

    apr.apr_tree.fill_tree_mean(apr, apr.apr_tree, input_particles, part_tree);
            ...
            for (z = 0; z < apr_iterator.spatial_index_z_max(level); z++) {
                for (x = 0; x < apr_iterator.spatial_index_x_max(level); ++x) {
                    for (apr_iterator.set_new_lzx(level, z, x); apr_iterator.global_index() < apr_iterator.end_index;
                        ...
                        temp_int = input_particles[apr_iterator];
...
#pragma omp parallel for schedule(dynamic) private(z, x) firstprivate(apr_tree_iterator)
        ...
        for (z = 0; z < apr.spatial_index_z_max(level); ++z) {
            ...
            for (x = 0; x < apr.spatial_index_x_max(level); ++x) {
                for (apr_iterator.set_new_lzx(level, z, x);
                    ...
                    output_particles[apr_iterator] = neigh_sum;//std::roundf(neigh_sum/(1.0f*pow((float)2*stencil_halves[0]+1, apr.apr_access.number_dimensions)));
                }
            }
        }
    ...
    apr.interp_img(recon_standard, output_particles);
