...
class APRTreeNumerics {
    ...
    static void fill_tree_mean_(APR& apr,ParticleData<S>& particle_data,ParticleData<U>& tree_data) {
...
#pragma omp parallel for schedule(dynamic) private(x_d, z_d) firstprivate(apr_iterator, parentIterator)
            ...
            for (z_d = 0; z_d < apr_iterator.spatial_index_z_max(level) / 2; z_d++) {
                for (int z = 2 * z_d; z <= std::min(2 * z_d + 1, (int) apr_iterator.spatial_index_z_max(level)); ++z) {
                    //the loop is bundled into blocks of 2, this prevents race conditions with OpenMP parents
                    for (x_d = 0; x_d < apr_iterator.spatial_index_x_max(level) / 2; ++x_d) {
                        for (int x = 2 * x_d; x <= std::min(2 * x_d + 1, (int) apr.spatial_index_x_max(level)); ++x) {
                            ...
                            //dealing with boundary conditions
                            float scale_factor_xz =
                                    (((2 * apr_iterator.spatial_index_x_max(level - 1) != apr_iterator.spatial_index_x_max(level)) &&
                                      ((x / 2) == (apr.spatial_index_x_max(level - 1) - 1))) +
                                     ((2 * apr_iterator.spatial_index_z_max(level - 1) != apr_iterator.spatial_index_z_max(level)) &&
                                      (z / 2) == (apr_iterator.spatial_index_z_max(level - 1) - 1))) * 2;
                            ...
                            for (apr_iterator.set_new_lzx(level, z, x);
                                ...
                                if (parentIterator.y() == (apr_iterator.spatial_index_y_max(level - 1) - 1)) {
                                    tree_data[parentIterator] =
                ...
                for (int z = 2 * z_d; z <= std::min(2 * z_d + 1, (int) apr.spatial_index_z_max(level)); ++z) {
                    ...
                    for (x_d = 0; x_d < apr.spatial_index_x_max(level) / 2; ++x_d) {
                        for (int x = 2 * x_d; x <= std::min(2 * x_d + 1, (int) apr.spatial_index_x_max(level)); ++x) {
                            ...
                            float scale_factor_xz =
                                    (((2 * apr_iterator.spatial_index_x_max(level - 1) != apr.spatial_index_x_max(level)) &&
                                      ((x / 2) == (apr_iterator.spatial_index_x_max(level - 1) - 1))) +
