...
void APRRaycaster::initObjects(int imageWidth, int imageHeight, float radius, float theta, float x0, float y0,
    ...
    cam.setPosition(glm::vec3(x0 + radius*sin(phi), y0 + radius * sin(theta)*cos(phi_s), z0 + radius * cos(theta)*cos(phi_s)));
    cam.setRotation(glm::fquat(1.0f, 0.0f, 0.0f, 0.0f));
    ...
    raytracedObject.setPosition(glm::vec3(0.0f, 0.0f, 0.0f));
    raytracedObject.setRotation(glm::fquat(1.0f, 0.0f, 0.0f, 0.0f));
    mvp = (*cam.getProjection()) * (*cam.getView());
}

void
APRRaycaster::getPos(int &dim1, int &dim2, float x_actual, float y_actual, float z_actual, int x_num, int y_num) {
    glm::vec2 pos = raytracedObject.worldToScreen(mvp, glm::vec3(x_actual, y_actual, z_actual), x_num, y_num);
    dim1 = round(-pos.y);
    dim2 = round(-pos.x);
}
