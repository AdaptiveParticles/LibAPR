    ...
    public:
        ...
        void read_apr(APR<ImageType>& apr,std::string file_name)
        {
           ...
           if(apr.particles_int_new.data.size()>0){
                ...
                hdf5_load_data_blosc(obj_id,hdf5_data_type,apr.particles_int_new.data.data(),name.c_str());
            }
...
//                apr_compress.decompress(apr,apr.particles_int);
        ...
        void write_apr(APR<ImageType>& apr,std::string save_loc,std::string file_name,APRCompress<ImageType>& apr_compressor,unsigned int blosc_comp_type = BLOSC_ZSTD,unsigned int blosc_comp_level = 2,unsigned int blosc_shuffle=1){
            ...
            if(compress_type_num > 0){
                ...
                //apr_compressor.compress(apr,particles_int_new);

            }

            dims = apr.particles_int_new.data.size();
            ...
            if(dims > 0){
                ...
                hdf5_write_data_blosc(obj_id, type, dataset_name.c_str(), rank, &dims, apr.particles_int_new.data.data(),blosc_comp_type,blosc_comp_level,blosc_shuffle);
        ...
        void write_apr_paraview(APR<ImageType>& apr,std::string save_loc,std::string file_name,ExtraParticleData<T>& parts){
            ...
            std::vector<uint16_t> xv;
            xv.resize(apr_iterator.total_number_parts());

            std::vector<uint16_t> yv;
            yv.resize(apr_iterator.total_number_parts());

            std::vector<uint16_t> zv;
            zv.resize(apr_iterator.total_number_parts());

            std::vector<uint8_t> levelv;
            levelv.resize(apr_iterator.total_number_parts());

            std::vector<uint8_t> typev;
            typev.resize(apr_iterator.total_number_parts());
...
#pragma omp parallel for schedule(static) private(particle_number) firstprivate(apr_iterator)
            ...
            write_main_paraview_xdmf_xml(save_loc,file_name,apr_iterator.total_number_parts());
