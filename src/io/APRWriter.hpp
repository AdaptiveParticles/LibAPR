...
//        H5Aclose(attr_id);
            ...
            apr.particles_int_old.initialize_structure_cells(apr.pc_data);
...
//        }
            ...
            for (int level = apr.level_min(); level <= apr.level_max(); ++level) {
                ...
                for (uint64_t particle_number = apr_iterator.particles_level_begin(level);
                    ...
                    apr_iterator(apr.particles_int_old) = Ip[level][counter];
            ...
            if (compress_type > 0) {
                ...
                apr_compress.decompress(apr, apr.particles_int_old);
        ...
        template<typename ImageType>
        void write_apr(APR<ImageType> &apr, std::string save_loc, std::string file_name,
                       APRCompress<ImageType> &apr_compressor, unsigned int blosc_comp_type = BLOSC_ZSTD,
                       unsigned int blosc_comp_level = 6, unsigned int blosc_shuffle = 1) {
            //
            //
            //  Bevan Cheeseman 2018
            //
            ...
            hdf5_write_attribute_blosc(pr_groupid, H5T_NATIVE_FLOAT, "quantization_factor", 1, dims_out,
                                       &quantization_factor);
            ...
            hdf5_write_attribute_blosc(pr_groupid, H5T_NATIVE_FLOAT, "sigma_th_max", 1, dims_out,
                                       &apr.parameters.sigma_th_max);
            ...
            hdf5_write_attribute_blosc(pr_groupid, H5T_NATIVE_FLOAT, "rel_error", 1, dims_out,
                                       &apr.parameters.rel_error);

            hdf5_write_attribute_blosc(pr_groupid, H5T_NATIVE_FLOAT, "noise_sd_estimate", 1, dims_out,
                                       &apr.parameters.noise_sd_estimate);

            hdf5_write_attribute_blosc(pr_groupid, H5T_NATIVE_FLOAT, "background_intensity_estimate", 1, dims_out,
            ...
            if (compress_type_num > 0) {
                ...
                temp_int.data = apr.particles_int_old.data;
	...
	#pragma omp parallel for default(shared) private(z_, x_, j_, node_val, status, y_coord) if(z_num_*x_num_ > 100)
        ...
        template<typename ImageType, typename T>
        void write_apr_paraview(APR<ImageType> &apr, std::string save_loc, std::string file_name,
                                ExtraPartCellData<T> &parts) {
            //
            //
            //  Bevan Cheeseman 2018
            //
            ...
            hdf5_write_data_blosc(obj_id, H5T_NATIVE_UINT16, name.c_str(), rank, &dims, Ip.data(), blosc_comp_type,
            ...
            name = "x";
            hdf5_write_data_blosc(obj_id, H5T_NATIVE_UINT16, name.c_str(), rank, &dims, xv.data(), blosc_comp_type,
                                  blosc_comp_level, blosc_shuffle);

            name = "y";
            hdf5_write_data_blosc(obj_id, H5T_NATIVE_UINT16, name.c_str(), rank, &dims, yv.data(), blosc_comp_type,
                                  blosc_comp_level, blosc_shuffle);

            name = "z";
            hdf5_write_data_blosc(obj_id, H5T_NATIVE_UINT16, name.c_str(), rank, &dims, zv.data(), blosc_comp_type,
                                  blosc_comp_level, blosc_shuffle);

            name = "level";
            hdf5_write_data_blosc(obj_id, H5T_NATIVE_UINT8, name.c_str(), rank, &dims, levelv.data(), blosc_comp_type,
                                  blosc_comp_level, blosc_shuffle);

            name = "type";
            hdf5_write_data_blosc(obj_id, H5T_NATIVE_UINT8, name.c_str(), rank, &dims, typev.data(), blosc_comp_type,
                                  blosc_comp_level, blosc_shuffle);
        ...
        void write_particles_only(APR<ImageType> apr, std::string save_loc, std::string file_name,
                                  ExtraPartCellData<S> &parts_extra) {
            //
            //
            //  Bevan Cheeseman 2018
            //
