...
#ifndef PARTPLAY_APR_ALT_ITERATOR_NEW_HPP
#define PARTPLAY_APR_ALT_ITERATOR_NEW_HPP

#include "APR.hpp"
#include "APRAccessAlt.hpp"
#include "APRTree.hpp"

template<typename ImageType>
class APRIteratorAlt {

protected:

    APRAccessAlt::LocalMapIterators local_iterators;
...
public:
    ...
    explicit APRIteratorAlt(APRAccessAlt& apr_access_){
    ...
    bool set_iterator_to_particle_by_number(const uint64_t particle_number){
        ...
        if(particle_number==0){
            ...
            set_gap_by_particle_cell(current_gap,current_particle_cell);
            ...
            current_particle_cell.y = ( current_gap.ptr->y_begin[current_gap.index]) + (uint16_t) (particle_number - (current_gap.ptr->global_index[current_gap.index]));
    ...
    inline uint64_t particles_offset_end(const uint16_t& level,const uint64_t& offset){
        ...
        if(apr_access->gap_map.data[level][offset].size() > 0){
            return  ( apr_access->gap_map.data[level][offset][0].global_index.back() + ( apr_access->gap_map.data[level][offset][0].y_end.back()- apr_access->gap_map.data[level][offset][0].y_begin.back()));
        } else {
            return 0;
        }
    ...
    bool set_neighbour_iterator(APRIteratorAlt<ImageType> &original_iterator, const uint8_t& direction, const uint8_t& index){
        //
...
protected:
    ...
    void set_gap_by_particle_cell(APRAccessAlt::MapIterator& it,ParticleCell& particleCell){
        it.index = 0;
        it.level = particleCell.level;
        it.pc_offset = particleCell.pc_offset;
        it.ptr = &((*apr_access).gap_map.data[particleCell.level][ particleCell.pc_offset][0]);
...
//            current_particle_cell.y = current_gap.iterator->first;
            set_gap_by_particle_cell(current_gap,current_particle_cell);
            current_particle_cell.global_index = current_gap.ptr->global_index[0];
            current_particle_cell.y = current_gap.ptr->y_begin[0];
    ...
    bool move_to_next_particle_cell(){
        ...
        if( (current_particle_cell.y+1) <= current_gap.ptr->y_end[current_gap.index]){
            //  Still in same y gap
            ...
            if(current_gap.index<(current_gap.ptr->y_begin.size())){
                //I am in the next gap
                current_particle_cell.global_index++;
                //current_particle_cell.y = current_gap.iterator->first; // the key is the first y value for the gap
                current_particle_cell.y = current_gap.ptr->y_begin[current_gap.index]; // the key is the first y value for the gap
                return true;
            } else {
                current_particle_cell.pc_offset++;
