...
#ifndef PARTPLAY_APRACCESSALT_HPP
#define PARTPLAY_APRACCESSALT_HPP
...
public:
    ...
    ExtraPartCellData<APRSparseRow> gap_map;
...
//        }
    ...
    template<typename T>
    void generate_from_apr(const APR<T> &apr) {
        //
        ...
        gap_map.depth_max = apr.level_max();
        gap_map.depth_min = apr.level_min();
        ...
        for(uint64_t i = gap_map.depth_min;i <= gap_map.depth_max;i++){
            gap_map.z_num[i] = apr.spatial_index_z_max(i);
        ...
        for (uint64_t i = (apr.level_min()); i <= apr.level_max(); i++) {
            ...
            for (z_ = 0; z_ < z_num_; z_++) {
                for (x_ = 0; x_ < x_num_; x_++) {
                    ...
                    if (apr.apr_access.gap_map.data[i][offset_pc_data].size() > 0) {
                        gap_map.data[i][offset_pc_data].resize(1);
                        for (auto const &element : apr.apr_access.gap_map.data[i][offset_pc_data][0].map) {
                            gap_map.data[i][offset_pc_data][0].y_begin.push_back(element.first);
                            gap_map.data[i][offset_pc_data][0].y_end.push_back(element.second.y_end);
                            gap_map.data[i][offset_pc_data][0].global_index.push_back(element.second.global_index_begin);
                        }
                        gap_map.data[i][offset_pc_data][0].y_0 = gap_map.data[i][offset_pc_data][0].y_begin[0];
                        gap_map.data[i][offset_pc_data][0].y_f = gap_map.data[i][offset_pc_data][0].y_end.back();
                    }
